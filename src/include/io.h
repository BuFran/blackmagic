...
#if defined(STM32F1)
INLINE
void io_setmode(uint32_t pin, const uint32_t mode)
{
	const uint32_t pinid = (io_pin(pin) < 8) ? (io_pin(pin)*4) : ((io_pin(pin)-8)*4);
	
	if (io_pin(pin) < 8) {
		GPIO_CRL(io_port(pin)) = (mode << pinid) |
			(GPIO_CRL(io_port(pin)) & ~(0x0f << pinid));
	}
	else {
		GPIO_CRH(io_port(pin)) = (mode << pinid) |
			(GPIO_CRH(io_port(pin)) & ~(0x0f << pinid));
	}
}
#endif
...
INLINE
void io_set(uint32_t pin, bool val)
{
	GPIO_BSRR(io_port(pin)) = (val) ? io_pin(pin) : (io_pin(pin) << 16);
}
...
{
#if defined(STM32F1)
	io_setmode(pin, GPIO_MODE_OUTPUT_50_MHZ | (GPIO_CNF_OUTPUT_PUSHPULL << 2));
#else
...
INLINE
void io_output_opendrain(uint32_t pin)
{
#if defined(STM32F1)
	io_setmode(pin, GPIO_MODE_OUTPUT_50_MHZ | (GPIO_CNF_OUTPUT_OPENDRAIN << 2));
#else
	gpio_mode_setup(io_port(pin), GPIO_MODE_OUTPUT, GPIO_PUPD_NONE, io_pin(pin));
#endif
...
{
#if defined(STM32F1)
	io_setmode(pin, GPIO_MODE_INPUT | (GPIO_CNF_INPUT_FLOAT << 2));
#else
...
#if defined(STM32F1)
	io_high(pin);
	io_setmode(pin, GPIO_MODE_INPUT | (GPIO_CNF_INPUT_PULL_UPDOWN << 2));
#else
	GPIO_MODER(io_port(pin)) = (GPIO_MODER(io_port(pin)) & ~GPIO_MODE_MASK(io_pinno(pin))) |
		GPIO_MODE(io_pinno(pin), GPIO_MODE_INPUT);
...
INLINE
void io_input_analog(uint32_t pin)
{
#if defined(STM32F1)
	io_setmode(pin, GPIO_MODE_INPUT | (GPIO_CNF_INPUT_ANALOG << 2));
#else
#endif
}
...
INLINE
void io_af_out(uint32_t pin)
{
#if defined(STM32F1)

	io_setmode(pin, GPIO_MODE_OUTPUT_50_MHZ | (GPIO_CNF_OUTPUT_ALTFN_PUSHPULL << 2));
#else
	/* no way ? */
#endif
}
...
INLINE
void io_af_out_opendrain(uint32_t pin)
{
#if defined(STM32F1)

	io_setmode(pin, GPIO_MODE_OUTPUT_50_MHZ | (GPIO_CNF_OUTPUT_ALTFN_OPENDRAIN << 2));
#else
	/* no way ? */
#endif
}
